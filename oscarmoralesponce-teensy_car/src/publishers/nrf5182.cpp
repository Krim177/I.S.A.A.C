...
static void nrfSendTask(void* args)
{
    ...
    while (true) {
      ...
      {  	
	    commonKnowledge.position_x = (int16_t) ldmap->position_x;
	    commonKnowledge.position_y = (int16_t) ldmap->position_y;
	    commonKnowledge.speed = ldmap->speed;
	    commonKnowledge.heading = ldmap->heading;
      }
      
//       commonKnowledge.clock = geClock();
      commonKnowledge.operationMode = getOperationMode();
      commonKnowledge.tioaState = getCooperativeState();  
...
//       Serial.print(extraMessageData.qrCode);
//       Serial.println(" NRF SEnd");
...
static void nrfRecvTask(void *param)
{
  ...
  while(1)
  {
	  ...
	  switch(rxState)
		...
		if (cksum == c)  // The message is accepted
		{
...
// 		      ldmap.clock = commonKnowledge->clock;
		      ...
		      ldmap.position_x = (float) commonKnowledge->position_x;
		      ldmap.position_y = (float) commonKnowledge->position_y;
