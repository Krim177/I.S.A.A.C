            ...
            if config.QR_code is None:
                ...
                # marker(s) were found
                    ...
                    if config.QR_code in ids.get().flatten():
                        ...
                        id_index, = np.where(flat == int(config.QR_code))
                    ...
                    print('[AC] sleeping for {0} seconds'.format(self.continual_capture_seconds))
                ...
                print('[AC tf] global x: {0}, z: {1}'.format(config.offset, config.distance))
