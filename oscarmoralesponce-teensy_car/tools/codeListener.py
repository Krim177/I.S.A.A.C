...
class codeListener(threading.Thread):
    ...
    def run(self):
        while True: # added this -Kelly
            #print("[CL] Code listener running")
    ...
    def recvBroadcastCode(self):
        ...
        try:
            ...
            if data is not None:
                ...
                print("\n[CL, rcb] Broadcast code received: ", self.code)
            ...
            print('[CL] no data received from broadcast')
    ...
    def writeCode(self):
        ...
        print("\n[CL wc] Broadcast code written to serial")
    ...
    def writeDistanceOffset(self):
        ...
        print("[CL wdo] Inputs: distance: {0}, offset: {1}".format(config.distance, config.offset))
        self.condition.release()
        #self.printSem.release()
        pay = self.payload(self.code, int(config.distance), int(config.offset), sensor1, sensor2, sensor3)
        print("[CL wdo] Writing payload to serial...")  # print payload
