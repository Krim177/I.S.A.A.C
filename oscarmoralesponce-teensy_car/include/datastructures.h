...
enum { INTERNAL_CONTROL, EXTERNAL_CONTROL};
...
#define SET_PARAM1ROS(rank, active, map) ((rank & 0xF) << 4) | (active & 0x7) << 1 | map 

#define GET_ACTIVEROS(param) (param  & 0x7) >> 1
#define GET_MAPROS(param) param & 1
...
//    uint32_t clock;
   ...
   uint8_t rssi;
   uint8_t propose:7;
   uint8_t isRotating:1;   //26 Bytes
   MessageData messageData;   // 9 Bytes
...
uint8_t onEndCarRobot(LDMap *ldmap, uint8_t *globalState, uint32_t clock, uint8_t code, int16_t distance,  int16_t angle);
void onInitCar();
