...
TaskHandle_t ISRHandle;

static SemaphoreHandle_t txComplete;
static QueueHandle_t eventHandler;


void dwmInterrupt(void* args)
{
    byte event;
    
    while(1) 
    {
	if (xQueueReceive(eventHandler, &event, portMAX_DELAY) == pdTRUE)
	{
// 	  Serial.print("Int");
// 	  Serial.println(event);
	  if (event & 1)
	    (*DW1000Class::_handleSent)();
//  	  if ((event & 2) && DW1000Class::_permanentReceive) {
//  	      DW1000Class::newReceive();
// // // 	      DW1000Class::setDefaults();
// 	      DW1000Class::startReceive();
// 	  }
	  if (event & 4)
	  ...
	  if (DW1000Class::_permanentReceive) {
 	      DW1000Class::newReceive();
// // 	      DW1000Class::setDefaults();
	      DW1000Class::startReceive();
	  }
...
void DW1000Class::begin(uint8_t irq, uint8_t rst) {
	...
	eventHandler = xQueueCreate(1, 1);
	xTaskCreate(dwmInterrupt, "ISR", configMINIMAL_STACK_SIZE + 20, NULL, 4, &ISRHandle);
// 	configAssert(ISRHandle);
...
void DW1000Class::handleInterrupt() {
       byte b = 0;
       BaseType_t xHigherPriorityTaken = pdFALSE;
	...
	if(isTransmitDone()) {
 	...
 	if(isReceiveFailed() || isReceiveTimeout()) {
	    b |= 2;
	}
	...
	clearAllStatus();
	if (b !=0)
	  xQueueSendFromISR(eventHandler, &b, &xHigherPriorityTaken);
 	portYIELD_FROM_ISR(xHigherPriorityTaken);
}
...
bool DW1000Class::isValid() {
// 	byte data[LEN_DEV_ID];
// 	data[0] = 0;
	uint32_t data;
	readBytes(DEV_ID, NO_SUB, (byte *)&data, LEN_DEV_ID);
	Serial.println(data, HEX);
	return data == 0xdeca0130;
...
void DW1000Class::readSystemEventStatusRegister() {
	readBytesISR(SYS_STATUS, NO_SUB, _sysstatus, LEN_SYS_STATUS);
}
...
void DW1000Class::getTransmitTimestampISR(DW1000Time& time) {
    byte txTimeBytes[LEN_TX_STAMP];
    readBytesISR(TX_TIME, TX_STAMP_SUB, txTimeBytes, LEN_TX_STAMP);
    time.setTimestamp(txTimeBytes);
}

void DW1000Class::getReceiveTimestampISR(DW1000Time& time) {
    byte rxTimeBytes[LEN_RX_STAMP];
    readBytesISR(RX_TIME, RX_STAMP_SUB, rxTimeBytes, LEN_RX_STAMP);
...
void DW1000Class::getTransmitTimestamp(dwTime_t* time) {
	readBytes(TX_TIME, TX_STAMP_SUB, time->raw, LEN_TX_STAMP);
}

void DW1000Class::getReceiveTimestamp(dwTime_t* time) {
	time->full = 0;
	readBytes(RX_TIME, RX_STAMP_SUB, time->raw, LEN_RX_STAMP);
	// correct timestamp (i.e. consider range bias)
	correctTimestamp(time);
}
...
void DW1000Class::setAntenaDelay(int64_t delay)
{
     DW1000Class::_antennaDelay.setTimestamp(delay);

}



void DW1000Class::correctTimestamp(dwTime_t* timestamp) {
	// base line dBm, which is -61, 2 dBm steps, total 18 data points (down to -95 dBm)
	float rxPowerBase = -(getReceivePower()+ 61.0f) * 0.5f;
	...
	adjustmentTime.full = (int)(rangeBias *DW1000Time::DISTANCE_OF_RADIO_INV * 0.001f);
	// apply correction
...
void DW1000Class::clearAllStatus() {
	...
	writeBytesISR(SYS_STATUS, NO_SUB, _sysstatus, LEN_SYS_STATUS);
}
...
void DW1000Class::readBytes(byte cmd, uint16_t offset, byte data[], uint16_t n) {
	...
	if (SPI.beginTransaction(*_currentSPI))
	{
	  digitalWrite(_ss, LOW);
 ...
 */
// TODO incomplete doc
void DW1000Class::readBytesISR(byte cmd, uint16_t offset, byte data[], uint16_t n) {
	byte header[3];
	uint8_t headerLen = 1;
	uint16_t i = 0;
	...
	if (SPI.beginTransactionISR(*_currentSPI))
	{
	  digitalWrite(_ss, LOW);
	  for(i = 0; i < headerLen; i++) {
		  SPI.transfer(header[i]); // send header
 ...
 */
// TODO offset really bigger than byte?
void DW1000Class::writeBytesISR(byte cmd, uint16_t offset, byte data[], uint16_t data_size) {
	byte header[3];
	uint8_t  headerLen = 1;
	uint16_t  i = 0;
