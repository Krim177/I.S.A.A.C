...
public:
	...
	*/
	...
	static void         getTransmitTimestampISR(DW1000Time& time);
	static void         getReceiveTimestampISR(DW1000Time& time);
	...
	static void         getReceiveTimestamp(dwTime_t* time);
	static void 	    getTransmitTimestamp(dwTime_t* time);
	static void 	    correctTimestamp(dwTime_t* timestamp);
...
//private:
	...
	static void readBytesISR(byte cmd, uint16_t offset, byte data[], uint16_t n);
	...
	static void writeBytesISR(byte cmd, uint16_t offset, byte data[], uint16_t n);
	...
	void setAntenaDelay(int64_t delay);
